    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            SELECT DISTINCT ON (object_id)
                object_id, lat, lon, altitude_m, vel_lat, vel_lon, vel_alt, confidence, time, source_cameras
            FROM object_tracks
...
async def get_object_history(object_id: str, minutes: int = 5) -> list[dict]:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT object_id, lat, lon, altitude_m, vel_lat, vel_lon, vel_alt, confidence, time, source_cameras
            FROM object_tracks
            WHERE object_id = $1
...
async def get_live_rays(within_seconds: int = 2) -> list[dict]:
    """Return the most recent ray per device within *within_seconds*."""
    pool = await get_pool()
    interval = datetime.timedelta(seconds=within_seconds)
    async with pool.acquire() as conn:
        rows = await conn.fetch(
