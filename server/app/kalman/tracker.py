...
class KalmanTracker:
    ...
    track before a new filter is created (spatial nearest-neighbour within
    ...
    async def update(self, correlated: CorrelatedDetection) -> None:
        ...
        matched = self._associate(correlated.lat, correlated.lon)
        if matched is not None:
            oid = matched
        else:
            self._next_id += 1
            oid = f"obj_{self._next_id:04d}"
            logger.debug("KalmanTracker: spawned new track %s at (%.6f, %.6f)", oid, correlated.lat, correlated.lon)
    ...
    def _associate(self, lat: float, lon: float) -> Optional[str]:
        """
